...
switch($_GET[act]){
  ...
  default:
      echo ' <article class="content static-tables-page">
                    <div class="title-block">
                        ...
                        <p class="title-description">   Edit Deskripsi Pencapaian Mata Pelajaran</p>
                    </div>
  ...
  echo "<div class='card'>
   ...
   $tampil=mysql_query("SELECT * FROM jadwal, mapel  
  ...
  case "editmapel":
	...
	<form id="formulir" class="form-horizontal" method="post" action='./modul/mod_mapel/aksi_mapel.php?module=mapel&act=update' role="form" parsley-validate>
  	...
  	<input type=hidden name=id_mapel value='<?php echo $r[id_mapel]; ?>'>
		...
		<label for="kode_mapel" class="col-sm-3 control-label">Kode Mapel</label>
		<div class="col-sm-2">
		<input type="text" name="kode_mapelxxx" class="form-control required" id="kode_mapel" placeholder="Kode Mapel" value="<?php echo $r[kode_mapel]; ?>" disabled>
		</div>
	</div>
  	...
  	<div class="form-group row" >
		<label for="nama_mapel" class="col-sm-3 control-label">Nama Mapel</label>
		<div class="col-sm-4">
		<input type="text" name="nama_mapelxxx" class="form-control required" id="nama_mapel" placeholder="Nama Mapel" value="<?php echo $r[nama_mapel]; ?>" disabled>
		</div>
	</div>
	
	<input type="hidden" name="kode_mapel" class="form-control" value="<?php echo $r[kode_mapel]; ?>">
	<input type="hidden" name="nama_mapel" class="form-control" value="<?php echo $r[nama_mapel]; ?>">
	<input type="hidden" name="tingkat" class="form-control" value="<?php echo $r[tingkat]; ?>">
	...
	<div class="form-group row" >
			<label for="tingkat" class="col-sm-3 control-label">Tingkat</label>
			<div class="col-sm-4">
			 <select name='tingkatxxx' class="form-control  wide required" disabled>
			 <option value=''> -- Pilih Tingkat --</option>
				<option value='AWALIYAH I' <?php if($r[tingkat]=="AWALIYAH I") { echo "selected=selected"; } else {echo "";} ?>> AWALIYAH I</option>
				  ...
				  <option value='WUSTHO I' <?php if($r[tingkat]=="WUSTHO II") { echo "selected=selected"; } else {echo "";} ?>> WUSTHO II</option>
